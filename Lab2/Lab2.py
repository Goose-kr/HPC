...
def vector_sum_cpu(A):
    ...
    for i in range(len(A)):
        count = count + A[i]
    running_time = Lab1.Lab1.toc()
    return running_time


def vector_sum_gpu(A):
    tensor = Lab1.Lab1.mat_copy_gpu(A)
    Lab1.Lab1.tic()
...
if __name__ == '__main__':
    ...
    while SIZE <= max_size:
        X = Lab1.Lab1.mat_generator(1, SIZE)
        cpu_timings.append(vector_sum_cpu(X))
        gpu_timings.append(vector_sum_gpu(X))
        iter_list.append(SIZE)
        SIZE = SIZE + step
    Lab1.Lab1.plotter(cpu_timings, gpu_timings, iter_list, label_x, label_y)
    print("done")